...
#Modified by me on 03/01/26

################################################################################
# NEW SECTION 7.5: Buyer Issues Invoice Acceptance Credential
################################################################################

echo -e "${YELLOW}[7.5/8] Buyer Accepts Invoice (Issues Acceptance Credential)...${NC}"
echo ""

BUYER_AGENT="tommyBuyerAgent"
SELLER_AGENT="jupiterSellerAgent"
ACCEPTANCE_REGISTRY_NAME="${BUYER_AGENT}_ACCEPTANCE_REGISTRY"

# Get the original invoice SAID
ORIGINAL_INVOICE_SAID=$(cat "./task-data/${SELLER_AGENT}-self-invoice-credential-info.json" | jq -r '.said')
INVOICE_SCHEMA_SAID=$(cat "./appconfig/schemaSaids.json" | jq -r '.invoiceSchema.said')

echo "  Original Invoice SAID: $ORIGINAL_INVOICE_SAID"
echo ""

# Create acceptance registry for buyer
echo -e "${BLUE}→ Creating invoice acceptance registry for buyer...${NC}"
./task-scripts/invoice/invoice-acceptance-registry-create.sh "$BUYER_AGENT" "$ACCEPTANCE_REGISTRY_NAME"

# Buyer issues acceptance credential
echo -e "${BLUE}→ Buyer issues invoice acceptance credential...${NC}"
./task-scripts/invoice/invoice-acceptance-issue.sh \
  "$BUYER_AGENT" \
  "$ORIGINAL_INVOICE_SAID" \
  "$INVOICE_SCHEMA_SAID"

echo -e "${GREEN}✓ Invoice acceptance credential issued by buyer${NC}"
echo ""

################################################################################
# NEW SECTION 7.6: Seller Admits Acceptance (Optional)
################################################################################

echo -e "${YELLOW}[7.6/8] Seller Receives Acceptance Notification...${NC}"
echo ""

# Send IPEX grant of acceptance from buyer to seller
echo -e "${BLUE}→ Sending acceptance credential to seller...${NC}"
./task-scripts/invoice/invoice-acceptance-grant.sh "$BUYER_AGENT" "$SELLER_AGENT"

# Seller admits the acceptance
echo -e "${BLUE}→ Seller admits acceptance credential...${NC}"
./task-scripts/invoice/invoice-acceptance-admit.sh "$SELLER_AGENT" "$BUYER_AGENT"

echo -e "${GREEN}✓ Invoice acceptance workflow complete${NC}"
echo -e "${GREEN}✓ Now you have: Invoice (Seller) → Acceptance (Buyer)${NC}"
echo ""
